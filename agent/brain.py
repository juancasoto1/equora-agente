    ...
    if telefono:
        ...
        if (await get_agent_modules(agent_id)).get("pedidos_voco", False):
            ...
            bloque_pv.append(
                ...
                "3. Pregunta la dirección de entrega y la forma de pago (transferencia/contra entrega).\n"
                "4. Con la confirmación del cliente, cierra el pedido con el marcador:\n"
                "   [[PEDIDO_CONFIRMAR:{\"direccion\":\"<dirección>\",\"notas\":\"<notas>\",\"costo_envio\":0,\"descuento\":0}]]\n"
                "   El marcador crea el pedido (estado 'creado'), borra el carrito y envía al\n"
                "   cliente la confirmación con número de pedido y total.\n\n"
                "CICLO DE VIDA DEL PEDIDO (solo informativo — el equipo lo gestiona en el panel):\n"
                "  creado → alistado → despachado → entregado (o cancelado)\n\n"
                "ESTADOS DE PAGO (solo informativo — el equipo los gestiona en el panel):\n"
                "  pendiente | pagado | cod (contra entrega)\n\n"
                "REGLAS:\n"
                "- NO uses [[PEDIDO:]] (ese es para Shopify) ni [[TIENDA:]] en este flujo.\n"
                "- [[PEDIDO_CONFIRMAR:]] solo cuando el cliente confirma explícitamente.\n"
                "- Si el cliente paga contra entrega, menciona ese acuerdo en 'notas'.\n"
                "- Si el cliente no da dirección, usa cadena vacía y avísale que el equipo la coordinará.\n"
                "- Después del marcador el carrito se borra; no lo menciones como activo.\n"
